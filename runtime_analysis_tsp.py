...
def try_strategies(instance, repeat= 1):
    ...
    for i in progressbar(range(repeat)):
        ...
        start = time()
        solution, dist_list = instance.solve(initial_strategy='random', local_search_strategy='2opt', log=True)
        sub_data.append(dist_list)
    ...
    strategies_labels  = ("Random-2opt",)#,"Hybride_EA/5/5")
...
for instance_name, instance_uri in zip(instances, instances_uris):
    ...
    for idx in range(len(strategies_labels)):
        ...
        plt.text(0.1, 0.9, f"Minimum Score: {np.mean(all_max)}")
