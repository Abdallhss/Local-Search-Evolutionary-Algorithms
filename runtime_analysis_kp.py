...
def try_strategies(instance, repeat= 2):
    data = list()
    for i in progressbar(range(repeat)):
        ...
        start = time()
        solution, _ = instance.random_thumwanit(max_p=0.1)
        profit, weight, feasible = instance.evaluate(solution)
        solution, weight, profit_list = instance.bits_flip_local_search(solution, weight, log=True)
    ...
    strategies_labels  = ("random_bitflip",)#,"Hybride_EA/5/5")
...
for instance_name, instance_uri in zip(instances, instances_uris):
    ...
    strategies_labels, data = try_strategies(instance, repeat=3)
